...
> - **RISC-V ISA-aware cipher selection (2026-07-10).** On a board with the vector-crypto extensions (`Zvkned`/`Zvkg`), OpenSSL AES-GCM is ≈6.4× faster than the ChaCha20 the stack was defaulting to. New opt-in, no-op-elsewhere APIs `TlsConfigBuilder::prefer_hardware_aead()` / `ClientBuilder::tls_prefer_hardware_aead()` detect this and prefer AES-GCM — a **RISC-V-specific optimization with zero impact on other platforms** (`#[cfg]`-gated). Measured single-connection gain: +6.8% @1 KB, **+128% (2.3×) @256 KB**; the ISA-aware client even exceeds a default-configured libcurl single-connection. Full data + honest scoping in `benchmark-results.md`.
