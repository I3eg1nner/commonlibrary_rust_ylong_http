...
- [x] 6.5 Certify ≥20% over **libcurl (the library)** — **NOT MET; fair result is PARITY.** After four rounds (micro-opts → concurrency → process-isolation → perf root-cause): in a fair test (proxy/origin in a **separate process**, no CPU contention) ylong **matches** libcurl across the board incl. 256 KB large body (+0.7%/+1.3%) and single connection. The earlier **−35% (large body) / −14% (concurrency) / +26% (K=8) / +41.5%** numbers were all **co-location/CPU-contention artifacts or `curl`-CLI comparisons — retracted.** ≥20% over libcurl is not achievable (both OpenSSL-bound, same cipher, equal instruction count).
- [x] 6.6 Correctness after the optimizations confirmed — 107 lib + 6 integration tests pass; clippy clean; async/sync × tls compile.
