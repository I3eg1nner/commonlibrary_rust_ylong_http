...
- **Platform-specific lever — RISC-V ISA-aware cipher selection (task 6.10).** The single connection *is* crypto-bound, and there the right lever is the ISA: on a RISC-V core with vector-AES (`Zvkned`), OpenSSL AES-GCM is ≈6.4× faster than the ChaCha20 the handshake was defaulting to. New opt-in APIs `TlsConfigBuilder::prefer_hardware_aead()` / `ClientBuilder::tls_prefer_hardware_aead()` detect the extension and prefer AES-GCM (measured +6.8% @1 KB, +128% @256 KB single-connection). **Design constraint honoured:** the detection is `#[cfg(all(target_os = "linux", target_arch = "riscv64"))]`-gated and opt-in, so it is a **pure no-op on non-RISC-V targets** — zero impact off-platform. This is the one place the data path (crypto) can be improved; it is orthogonal to the I/O-scheduling lever above.
