...
pub(crate) async fn tcp_stream(eyeballs: HappyEyeballs) -> Result<TcpStream, HttpClientError> {
    eyeballs
        .connect()
        .await
        .map_err(|e| {
...
mod tls {
    ...
    use crate::async_impl::proxy::{HttpProxyTunnel, HttpsProxyTunnel, ProxyTunnel, TunnelConnect};
    #[cfg(feature = "http3")]
    ...
    /// Describes the proxy hop for an HTTPS origin connection: whether a proxy
    /// is used, and if so whether the proxy itself is TLS-secured.
    enum ProxyKind {
        /// No proxy; connect directly to the origin.
        Direct,
        /// Plaintext (HTTP) proxy: CONNECT tunnel over plain TCP.
        Http,
        /// TLS-secured (HTTPS) proxy: TLS to the proxy, then CONNECT over it.
        Https {
            config: TlsConfig,
            proxy_host: String,
        },
    ...
    async fn https_connect(
        ...
        // Establish the transport to the origin: direct, plaintext-proxy tunnel,
        // or TLS-secured-proxy tunnel (TLS-in-TLS). The origin TLS layer wraps the
        // resulting `ProxyTunnel` uniformly.
        let is_proxy = !matches!(proxy_kind, ProxyKind::Direct);
